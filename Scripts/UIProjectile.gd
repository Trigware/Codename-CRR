...
const projectile_modulate_tween_duration = 0.5
...
func on_projectile_init():
	...
	alpha_mod = 0
	missing_texture.empty_tiles_rate = 0.0 if is_corrupted else missing_texture_normal_rate
...
func handle_hitting_monitor_bounds():
	...
	if not has_hit or not can_explode: return
...
func move_projectile(delta: float):
	...
	position += pos_delta
	projectile_traveled_distance += Help.pyth(pos_delta.x, pos_delta.y)
